...
const SingleProduct = () => {
  ...
  const viewedProducts = useSelector(
    (state) => state.fetchViewedProducts.viewedProducts,
  );
  ...
  const handleAddToCart = (e) => {
    ...
    const itemInCart = cart?.find(
      ({ instance }) => instance._id === singleItem._id,
    );
    if (itemInCart) {
    ...
    async function fetchData() {
      try {
        const response = await axios.get(
          `http://127.0.0.1:4000/api/products/${localStorage.getItem(
            "viewedProducts",
          )}`,
        );
  ...
  useEffect(() => {
    if (localStorage.getItem("viewedProducts")) {
      ...
      const addViewedProduct = async () => {
        try {
          await axios.post(`http://127.0.0.1:4000/api/last-viewed-products`, {
