...
const SingleOrder = () => {
    ...
    useEffect(() => {
        axios.get(`http://127.0.0.1:4000/api/orders/${currentOrderId}`, {
            headers: {
                "Authorization": token,
            }
        })
    ...
    const handleShipment = () => {
        ...
        axios.put(`http://127.0.0.1:4000/api/orders/${currentOrderId}`, currentOrder, {
            headers: {
                "Authorization": token,
            }
        })
                    ...
                    <ul className={styles.SingleOrderProducts}>
                        {
                            currentOrder?.products?.map((product, i) => (
                                <li key={product._id} className={styles.SingleOrderProductsItem}>
                                    ...
                                    <input type="checkbox" onChange={handleCheck} disabled={currentOrder.completed} />
                                </li>
                            ))
                        }
                    </ul>
